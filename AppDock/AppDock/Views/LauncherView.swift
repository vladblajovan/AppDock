...
struct LauncherView: View {
        ...
        ScrollView(.vertical, showsIndicators: false) {
            VStack(alignment: .leading, spacing: PlatformStyle.sectionSpacing) {
                ...
                HStack {
                    ...
                    HStack(spacing: 4) {
                        viewModeButton(.folders, symbol: "square.grid.2x2")
                        viewModeButton(.list, symbol: "list.bullet")
                    }
                }
                .padding(.horizontal, PlatformStyle.panelPadding)
    ...
    private func viewModeButton(_ mode: AppViewMode, symbol: String) -> some View {
        Button {
            ...
            Image(systemName: symbol)
                ...
                .foregroundStyle(viewModel.viewMode == mode ? Color.accentColor : .secondary)
                .background(
                    RoundedRectangle(cornerRadius: 6)
                        .fill(viewModel.viewMode == mode ? Color.accentColor.opacity(0.15) : Color.clear)
                )
        }
        .buttonStyle(.plain)
    }
